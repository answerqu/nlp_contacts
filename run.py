...
from avito.text import TextPreprocessor
...
class Test:
    ...
    def train(self) -> pd.DataFrame:
        ...
        if not os.path.exists(path):
            self.logger.info(f'Файл {path} не найден. Запускаем скрипт get_train_data.sh')
            os.system('bash ' + os.path.join(self.data_dir, 'get_train_data.sh'))
            if not os.path.exists(path):
    ...
    def process(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        self.logger.info(f'Инициализация модели.')
        model = joblib.load('lib/models/model.pkl')

        task1_prediction = pd.DataFrame(columns=['index', 'prediction'])
        task1_prediction['index'] = test.index
        
        preprocessor = TextPreprocessor(max_len=400)
        test = preprocessor.full_text_preprocessing(test)
        task1_prediction['prediction'] =  model.predict_proba(test).T[1]
